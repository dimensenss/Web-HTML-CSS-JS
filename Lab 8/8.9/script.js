...
function statistic(){
    ...
    document.getElementById("arr_statistic").innerHTML = 'Середнє арифметичне: '+average;
}
function modify(){
...
function check(){
    document.getElementById("arr_check").innerHTML ="Кожен елмент масиву менше 20 "+`${array.every(checkEl)}`+" ------ Хоча б один елемент менше 20 " + `${array.some(checkEl)}`;
}

function findEl(element){
    element = Math.floor(element);
    if(-5<element&&element<5){
        return true;
    }
    else{
        return false;
...
function find(){
    document.getElementById("arr_find").innerHTML = `${array.find(findEl)}`;
}