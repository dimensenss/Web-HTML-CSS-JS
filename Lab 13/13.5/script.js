...
xhttp.open("GET","/files/data.json", true);
xhttp.send();
var stud = [];
  xhttp.onreadystatechange = function() {
    if (xhttp.status == 200 && xhttp.readyState == 4) {
      ...
      var st=JSON.parse(text);
          for(var i = 0; i < 5; i++)
            {
              stud.push(new Student(st.name[i], st.faculty[i], st.course[i], st.group[i]))
            }
...
var lesson = [];
  xhttp.onreadystatechange = function() {
    if (xhttp.status == 200 && xhttp.readyState == 4) {
      var text = xhttp.responseText;
      var les=JSON.parse(text);
          for(var i = 0; i < 3; i++)
            {
              lesson.push(new Lesson(les.lesson[i], les.hours[i], les.teacher[i]))
            }
    ...
    var exam = [];
      xhttp.onreadystatechange = function() {
        if (xhttp.status == 200 && xhttp.readyState == 4) {
          ...
          for(var i = 0; i < 6; i++)
            {
              exam.push(new Exam(ex.form[i], ex.mark[i], ex.date[i], ex.i_st[i], ex.i_les[i]))
            }
