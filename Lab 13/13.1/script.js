...
xhttp.open("GET","/files/data", true);
xhttp.send();
var stud = [];
  xhttp.onreadystatechange = function() {
    if (xhttp.status == 200 && xhttp.readyState == 4) {
      ...
      for(var i = 0; i < 5; i++)
        {
          stud.push(new Student(text[i], text[i+5], text[i+10], text[i+15]))
        }
      ...
      for(var i = 0; i < stud.length; i++){
        ...
        td.textContent = stud[i].name + " " + stud[i].faculty + " "+  stud[i].course+ "c. "+  stud[i].group;
        tr.appendChild(td);
  ...
  var lesson = [];
    xhttp.onreadystatechange = function() {
      if (xhttp.status == 200 && xhttp.readyState == 4) {
        ...
        for(var i = 0; i < 3; i++)
          {
            lesson.push(new Lesson(text[i+20], text[i+23], text[i+26]))
          }
        ...
        for(var i = 0; i < lesson.length; i++){
          var tr = document.createElement("tr");
          var td = document.createElement("td");
          td.textContent = lesson[i].lesson + " " + lesson[i].count_of_hours + " h./w. "+  lesson[i].teacher;
          tr.appendChild(td);
          table.appendChild(tr);}
    ...
    var exam = [];
      xhttp.onreadystatechange = function() {
        if (xhttp.status == 200 && xhttp.readyState == 4) {
          ...
          for(var i = 0; i < 6; i++)
            {
              exam.push(new Exam(text[i+29], text[i+35], text[i+41], text[i+47], text[i+53]))
            }
          ...
          for(var i = 0; i < exam.length; i++){
            var tr = document.createElement("tr");
            var td = document.createElement("td");
            td.textContent = exam[i].form_of_exam + " " + exam[i].mark + " "+  exam[i].date+ " stud-"+  exam[i].index_st+ " les-"+  exam[i].index_l;
            tr.appendChild(td);
            table.appendChild(tr);}
