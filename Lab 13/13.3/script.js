...
xhttp.open("GET","/files/data3.xml", true);
xhttp.send();
var stud = [];
  xhttp.onreadystatechange = function() {
    if (xhttp.status == 200 && xhttp.readyState == 4) {
      ...
      var st=textInf.getElementsByTagName("Student");
          for(var i = 0; i < 5; i++)
            {
              stud.push(new Student(st[i].getAttribute("name"), st[i].getAttribute("faculty"), st[i].getAttribute("course"), st[i].getAttribute("group")))
            }
  ...
  xhttp.onreadystatechange = function() {
    if (xhttp.status == 200 && xhttp.readyState == 4) {
      ...
      var les=textInf.getElementsByTagName("Lesson");
          for(var i = 0; i < 3; i++)
            {
              lesson.push(new Lesson(les[i].getAttribute("lesson"), les[i].getAttribute("hours"), les[i].getAttribute("teacher")))
            }
    ...
    var exam = [];
      xhttp.onreadystatechange = function() {
        if (xhttp.status == 200 && xhttp.readyState == 4) {
          ...
          for(var i = 0; i < 6; i++)
            {
              exam.push(new Exam(ex[i].getAttribute("form"), ex[i].getAttribute("mark"), ex[i].getAttribute("date"), ex[i].getAttribute("i_st"), ex[i].getAttribute("i_l")))
            }
